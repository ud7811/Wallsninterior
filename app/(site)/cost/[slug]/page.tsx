...
export default async function CostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><StructuredData data={[articleSchema(item.h1, item.seo.description, `/cost/${item.slug}`, item.updatedAt), faqSchema(item.faqs), breadcrumbsSchema(crumbs)]} /><article>
    ...
    <section className="content-section"><h2>Price at a glance</h2><div className="table-wrap"><table><thead><tr><th>Specification</th><th>Indicative range</th><th>Best for</th></tr></thead><tbody>{item.summaryTable.map(row => <tr key={row.label}><td data-label="Specification">{row.label}</td><td data-label="Indicative range" className="number">{formatINR(row.range.min)} – {formatINR(row.range.max)}</td><td data-label="Best for">{row.note}</td></tr>)}</tbody></table></div></section>

...
export function CtaBanner({ heading, subheading }: { heading: string; subheading?: string }) {
  return <section className="cta-banner"><div><p className="eyebrow light">Clear scope. Clear price.</p><h2>{heading}</h2>{subheading && <p>{subheading}</p>}</div><div className="button-row"><Link className="button button-light" data-cta="cta-banner-form" href="/contact">Get free quote</Link><WhatsAppLink className="button button-whatsapp" ctaId="cta-banner-whatsapp" fallback={siteConfig.whatsappMessages.ctaBanner}>WhatsApp us</WhatsAppLink></div></section>
}

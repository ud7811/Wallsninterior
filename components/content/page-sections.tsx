...
export function CtaBanner({ heading, subheading }: { heading: string; subheading?: string }) {
  return <section className="cta-banner"><div><p className="eyebrow light">Clear scope. Clear price.</p><h2>{heading}</h2>{subheading && <p>{subheading}</p>}</div><div className="button-row"><Link className="button button-light" data-cta="cta-banner-form" href="/contact">Get free quote</Link><a className="button button-whatsapp" data-cta="cta-banner-whatsapp" href={buildWhatsAppLink({ number: siteConfig.whatsapp, text: siteConfig.whatsappMessages.ctaBanner })} target="_blank" rel="noreferrer"><WhatsAppIcon />WhatsApp us</a></div></section>
}

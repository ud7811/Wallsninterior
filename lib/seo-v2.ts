...
export function buildMetadata({ seo, path, images, noindex = false }: { seo: SeoBlock; path: string; images?: ImageAsset[]; noindex?: boolean }): Metadata {
  ...
  const image = images?.find((item) => item.isReal)?.src ?? seo.ogImage ?? "/images/og-default.jpg"
  return {
    ...
    openGraph: { title: seo.title, description: seo.description, url, siteName: siteConfig.name, locale: "en_IN", type: "website", images: [image] },
